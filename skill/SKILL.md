...
### إيقاع السرعة حسب النوع
| نوع المحتوى | طول القطعة المثالي | تعليق |
|-------------|-------------------|--------|
| Montage حماسي | 1-3 ثواني لكل قطعة | قص سريع على نبض الموسيقى |
| محتوى تعليمي/شرح | 5-10 ثواني | متابعة سهلة |
| فيديو استرخاء/سينمائي | 10-30 ثانية | لقطات طويلة وهادئة |
| ريلز/شورتس سريعة | 2-4 ثواني | إيقاع سريع جداً |

### مونتاج على إيقاع الموسيقى (Beat Sync)
```markdown
الفكرة: تقطع الفيديو على ضربات (beats) الموسيقى بحيث يظهر كل قص مع نبضة.

كيف تعرف مكان الضربات:
- الموسيقى غالباً 120-128 BPM = ضربة كل 0.5 ثانية تقريباً
- أو استخدم أداة BPM detector
- قص على: ضربة كل 2 أو 4 (أي الذروات)

مثال: عند 120 BPM، الضربة كل 0.5 ث، فتقص كل 1 أو 2 ثانية
```

### قواعد القص الذكية
```markdown
1. ❌ لا تقص منتصف حركة - انتظر اكتمالها أو بدايتها
2. ❌ لا تقص منتصف كلمة - قطع الحديث يفقد المعنى
3. ✅ قص عند تغير الزاوية أو حركة كبيرة
4. ✅ قص عند نبضة موسيقية (نبض الفيديو مع الموسيقى)
5. ✅ اترك "مسافة تنفس" (breathing room) بعد الذروة
6. قص على الحركة (match cut): اربط مشهد بآخر بحركة متشابهة
```
...
### اختيار نوع الانتقال حسب المزاج
| الانتقال | متى تستخدمه |
|----------|-------------|
| **Fade** (تلاشي) | بداية ونهاية الفيديو، مشاهد هادئة، انتقال زمني |
| **Dissolve** (ذوبان) | مشاهد متشابهة، مرور الوقت، مزاج حالم |
| **Wipe** (مسح) | تغيير مكان كلي، مشاهد متقابلة |
| **Slide** (انزلاق) | تغيير مشهد سريع، فن قصصي |
| **Circle** (دائرة) | تركيز على تفصيلة، zoom in لتفصيلة |
| **Zoom/Push** | تشديد على لحظة، شعور بالقرب |
| **Flash/فائدة Light** (وميض) | صدمة، انفجار، انتقال مفاجئ |
| **قطع جاف (Hard cut)** | معظم المونتاج السريع - الأكثر طبيعية |

### القاعدة الذهبية للانتقالات
```markdown
- أغلب الوقت: استخدم القطع الجاف (Hard cut) - الأكثر احترافاً
- الانتقالات المزخرفة (wipe, circle): للحظات قليلة فقط كتأثير خاص
- لا تبالغ في الانتقالات - تظهر الهواية
- طول الانتقال الأمثل: 0.3 - 1 ثانية
```

### متى تضيف المؤثرات الصوتية (SFX)
```markdown
SFX يعزز اللحظة ولا يستبدلها. ضعها في:
- لحظة الظهور (popping on screen) -> Pop/Click
- نقلت أنظارك لمفاجأة -> Whoosh
- لحظة ضعف/فشل -> Fail sounds
- لحظة انتصار -> Epic Boom
- لحظة صمت مرعبة -> Suspense

قاعدة: SFX واحد واضح أفضل من عدة متداخلة
```
...
### قالب التخطيط (Plan First)
```markdown
500أقترح دائماً خطة قبل تنفيذ أي مونتاج:

1. **الفيديو**: المدة، الدقة، المحتوى الأساسي
2. **المزاج**: مثير/هادئ/مضحك...
3. **الهيكل**: 3 ثواني افتتاحية، الجسد، الختام
4. **الموسيقى**: موسيقى كاملة؟ متى ترتفع/تنخفض
5. **SFX**: 3-5 نقاط رئيسية بوضوح
6. **النصوص**: العنوان، الترجمة، المواضع
7. **الأفكتات/الانتقالات**: ما النوع ومتى
8. **الثومبنيل**: صورة + عنوان
9. **التصدير**: الدقة النهائية والجودة
